function process(ctx) {
  if (!ctx || ctx.phase !== "request" || !ctx.url) return;

  function bytesToText(bytes) { try { return Anywhere.codec.utf8.decode(bytes || new Uint8Array()); } catch (_) { return ""; } }
  function textToBytes(text) { return Anywhere.codec.utf8.encode(String(text == null ? "" : text)); }
  function toBytes(value) {
    ...
    try { return new Uint8Array(value); } catch (_) { return textToBytes(String(value)); }
  }
  function headersToObject(headers) {
    ...
    if (Array.isArray(headers)) {
      for (var i = 0; i < headers.length; i++) {
        var item = headers[i];
        if (item && item.length >= 2) object[String(item[0])] = String(item[1]);
      }
    } else {
      for (var key in headers) object[key] = String(headers[key]);
    }
    return object;
  }
  function headersToArray(headers) {
    ...
    var array = [];
    for (var key in headers) if (headers[key] !== void 0) array.push([String(key), String(headers[key])]);
    return array;
  }
  ...
  var $request = {
    ...
    headers: headersToObject(ctx.requestHeaders || ctx.reqHeaders || ctx.headers),
    body: bytesToText(ctx.body),
    bodyBytes: toBytes(ctx.body)
  };

  // 让原脚本进入 Surge 分支，使用 $persistentStore / $httpClient / $done 兼容层。
  var $environment = { "surge-version": "Anywhere", "stash-version": "" };
  var $script = { startTime: Date.now() / 1000 };
  var $notification = { post: function () {} };
  var $notify = function () {};

  var $persistentStore = {
    read: function (key) {
      try { var value = Anywhere.store.getString(key); return value == null ? null : value; } catch (_) { return null; }
    },
    write: function (value, key) {
      try { Anywhere.store.set(key, textToBytes(value), true); return true; } catch (_) { return false; }
    }
  };
  var $prefs = {
    ...
    removeValueForKey: function (key) { try { Anywhere.store.remove(key); return true; } catch (_) { return false; } },
  ...
  function httpRequest(method, options, callback) {
    try {
      ...
      var body = options.bodyBytes || options.body || options.data;
      if (!url) { callback(new Error("Missing request URL")); return; }
      if (typeof fetch !== "function") { callback(new Error("fetch is not available in Anywhere runtime")); return; }
      fetch(url, { method: method, headers: headers, body: body }).then(function (response) {
        var responseHeaders = {};
        if (response.headers && response.headers.forEach) response.headers.forEach(function (value, key) { responseHeaders[key] = value; });
        return response.arrayBuffer().then(function (arrayBuffer) {
          var bytes = new Uint8Array(arrayBuffer);
          var text = "";
          try { text = Anywhere.codec.utf8.decode(bytes); } catch (_) {}
          callback(null, { status: response.status, statusCode: response.status, headers: responseHeaders, body: text, bodyBytes: bytes }, text);
        });
  ...
  var $httpClient = {
    get: function (options, callback) { httpRequest("GET", options, callback); },
    post: function (options, callback) { httpRequest("POST", options, callback); },
    put: function (options, callback) { httpRequest("PUT", options, callback); },
    delete: function (options, callback) { httpRequest("DELETE", options, callback); }
  };
  var $task = { fetch: function (options) {
    return new Promise(function (resolve, reject) {
      httpRequest((options && options.method) || "GET", options, function (error, response, body) {
        if (error) reject(error);
        else resolve({ status: response.status, statusCode: response.statusCode, headers: response.headers, body: body, bodyBytes: response.bodyBytes });
      });
    });
  } };
  ...
  function $done(value) {
    ...
    try {
      ...
      var headers = response.headers || value.headers || [];
      var status = normalizeStatus(response.status || value.status || response.statusCode || value.statusCode || 200);
      var bodyValue = response.bodyBytes !== void 0 ? response.bodyBytes :
        value.bodyBytes !== void 0 ? value.bodyBytes :
        response.body !== void 0 ? response.body :
        value.body !== void 0 ? value.body : void 0;
      ctx.status = status;
      ctx.headers = headersToArray(headers);
      if (bodyValue !== void 0) ctx.body = toBytes(bodyValue);
      Anywhere.done();
    } catch (error) {
      Anywhere.respond({ status: 500, headers: [["content-type", "text/plain; charset=utf-8"]], body: String(error && error.stack || error) });
    }
  }
